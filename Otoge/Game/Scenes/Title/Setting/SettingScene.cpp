...
SettingScene::SettingScene() : Scene("SettingScene", 40.f, 100.f)
{
    ...
    CloseButton_ = std::make_shared<Button>("< Close", ScreenData(0.f, 0.f, 20.f, 100.f),
                                            TitleBar_.lock()->GetDefaultScaler());
    ...
    auto l_TitleLabel = std::make_shared<Label>("設定", ScreenData(0.0f, 0.0f, 100.f, CloseButton_->GetScreenHeight()),
                                                TitleBar_.lock()->GetDefaultScaler());
    ...
    BodyPanel_ = std::make_shared<ScrollablePanel>("bodypanel", ScreenData(3.f, 5.f, 94.f, 95.f),
                                                   ScreenData(0.f, 0.f, 100.f, 190.f), DefaultScaler_);
    ...
    {
        DisplaySectionLabel_ = std::make_shared<Label>("ディスプレイ", ScreenData(0.f, 0.f, 100.f, 2.5f),
                                                       BodyPanel_->GetPanelInstance()->GetDefaultScaler());
        ...
        {
            WindowSizeDescription_ = std::make_shared<Label>(
                "解像度:", ScreenData(0.f, DisplaySectionLabel_->GetScreenHeight(), 20.f, 1.5f),
                BodyPanel_->GetPanelInstance()->GetDefaultScaler());
            ...
            WindowSizeList_ = std::make_shared<DropdownList>("WindowSizeList",
                                                             ScreenData(WindowSizeDescription_->GetScreenWidth(),
                                                                        WindowSizeDescription_->GetPositionY(), 20.f,
                                                                        WindowSizeDescription_->GetScreenHeight() +
                                                                        0.0f), AllowWindowSizes_.size(),
                                                             BodyPanel_->GetPanelInstance()->GetDefaultScaler());
            ...
            for(l_ItemCount = 0; l_ItemCount < AllowWindowSizes_.size(); l_ItemCount++)
            {
                ...
                WindowSizeList_->AddItem(l_ItemCount, l_WindowSize,
                                         std::vector<std::string>{ l_WindowWidth, l_WindowHeight });
            }
        }
    }
    FullscreenCheck_ = std::make_shared<CheckBox>(
        "フルスクリーン", ScreenData(WindowSizeList_->GetPositionX() + WindowSizeList_->GetScreenWidth() + 1.5f,
                              WindowSizeDescription_->GetPositionY(), 25.f, 1.5f),
        BodyPanel_->GetPanelInstance()->GetDefaultScaler());
    BodyPanel_->GetPanelInstance()->AddChildTask(FullscreenCheck_);
    VSyncCheck_ = std::make_shared<CheckBox>(
        "垂直同期", ScreenData(WindowSizeList_->GetPositionX(),
                           WindowSizeDescription_->GetPositionY() + WindowSizeDescription_->GetScreenHeight() + 1.5f,
                           20.f, 1.5f), BodyPanel_->GetPanelInstance()->GetDefaultScaler());
...
void SettingScene::OnStartedFadeIn()
{
    ...
    // 解像度
    auto l_CurrentSize = std::find_if(AllowWindowSizes_.begin(), AllowWindowSizes_.end(),
...
{
    if(CloseButton_->IsClickedMouse() || (!IsOnMouse() && MouseManager::GetInstance()->IsDownButton(MOUSE_INPUT_LEFT) &&
        IsEnable()))
    {
        StartFadeOut();
    }
    if(KeyboardManager::GetInstance()->IsHoldKey(KEY_INPUT_LCONTROL) && KeyboardManager::GetInstance()->
        IsDownKey(KEY_INPUT_O))
    {
        StartFadeIn();
    }
...
void SettingScene::Draw()
{
    ...
    DrawBox(engine::CastToInt(l_FixedContentField.posX), engine::CastToInt(l_FixedContentField.posY),
            engine::CastToInt(l_FixedContentField.width), engine::CastToInt(l_FixedContentField.height),
            BodyPanel_->baseColor, TRUE);
}
